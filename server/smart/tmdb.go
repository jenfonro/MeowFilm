...
func embyTMDBSearchMulti(database *db.DB, query string) ([]embyTMDBSearchItem, error) {
	...
	apiBase := tmdb.ResolveAPIBase(database)
	...
	searchOnce := func(query string) ([]embyTMDBSearchItem, error) {
		...
		doTV := func(language string) ([]embyTMDBSearchItem, error) {
			u, _ := url.Parse(tmdb.JoinAPI(apiBase, "search/tv"))
			params := u.Query()
			params.Set("query", qq)
			params.Set("page", "1")
			params.Set("include_adult", tmdb.BoolToStr(includeAdult))
		...
		doMovie := func(language string) ([]embyTMDBSearchItem, error) {
			u, _ := url.Parse(tmdb.JoinAPI(apiBase, "search/movie"))
			params := u.Query()
			params.Set("query", qq)
			params.Set("page", "1")
			params.Set("include_adult", tmdb.BoolToStr(includeAdult))
		...
		doReq := func(language string, region string) ([]embyTMDBSearchItem, error) {
			u, _ := url.Parse(tmdb.JoinAPI(apiBase, "search/multi"))
			params := u.Query()
			params.Set("query", qq)
			params.Set("page", "1")
			params.Set("include_adult", tmdb.BoolToStr(includeAdult))
			...
			var data tmdb.MultiSearchResponse
			if err := json.NewDecoder(resp.Body).Decode(&data); err != nil {
				return nil, err
			}
...
func embyTMDBDiscover(database *db.DB, mediaType string, yearStart int, yearEnd int, sortBy string, page int) (items []embyTMDBSearchItem, total int, err error) {
	...
	endpoint := tmdb.JoinAPI(apiBase, "discover/"+mt)
...
func embyTMDBGetTVDetailUpstream(database *db.DB, tmdbID int) (*embyTMDBTVDetail, error) {
	...
	u, _ := url.Parse(tmdb.JoinAPI(apiBase, fmt.Sprintf("tv/%d", id)))
	params := u.Query()
	if lang != "" {
	...
	seasons := make([]TMDBSeason, 0, len(data.Seasons))
	for _, s := range data.Seasons {
	...
	// Compute latest aired season/episode deterministically from TMDB seasons + season detail.
	// Do not rely on last_episode_to_air, which can lag behind or be inconsistent.
	latestSeason := 0
	latestEpisode := 0
	ended := strings.EqualFold(strings.TrimSpace(data.Status), "Ended")
	now0 := time.Now()
	// 1) Pick latest season that has started airing (CN day).
	for _, s := range data.Seasons {
		if s.SeasonNumber <= 0 || s.EpisodeCount <= 0 {
			continue
		}
		if strings.TrimSpace(s.AirDate) == "" {
			// If TMDB doesn't provide air_date for a season, keep it eligible only when ended.
			if !ended {
				continue
			}
		} else if !embyIsAiredDate(s.AirDate, now0) {
			continue
		}
		...
		if seasonCount > 0 {
			...
			if eps, err := embyTMDBGetTVSeasonEpisodesAtLeast(database, id, latestSeason, seasonCount); err == nil && len(eps) > 0 {
				for _, e := range eps {
					...
					if ad == "" {
						if ended {
							if e.Episode > maxAired {
								maxAired = e.Episode
							}
						}
						continue
					}
					if !embyIsAiredDate(ad, now0) {
						continue
					}
					if e.Episode > maxAired {
						maxAired = e.Episode
...
func embyTMDBGetTVSeasonDetail(database *db.DB, tmdbID int, season int) (*embyTMDBTVSeasonDetail, error) {
	...
	return embyTMDBGetTVSeasonDetailAtLeast(database, tmdbID, season, 0)
}
...
func embyTMDBGetTVSeasonDetailAtLeastUpstream(database *db.DB, tmdbID int, season int, minEpisodes int) (*embyTMDBTVSeasonDetail, error) {
	...
	apiBase := tmdb.ResolveAPIBase(database)
	u, _ := url.Parse(tmdb.JoinAPI(apiBase, fmt.Sprintf("tv/%d/season/%d", tmdbID, season)))
	params := u.Query()
	if lang != "" {
		params.Set("language", lang)
	}
	...
	out := make([]TMDBSeasonEpisode, 0, len(raw.Episodes))
	for _, e := range raw.Episodes {
		if e.EpisodeNumber <= 0 {
			continue
		}
		out = append(out, TMDBSeasonEpisode{
...
func embyTMDBGetTVSeasonEpisodes(database *db.DB, tmdbID int, season int) ([]TMDBSeasonEpisode, error) {
	return embyTMDBGetTVSeasonEpisodesAtLeast(database, tmdbID, season, 0)
}

func embyTMDBGetTVSeasonEpisodesAtLeast(database *db.DB, tmdbID int, season int, minEpisodes int) ([]TMDBSeasonEpisode, error) {
	if tmdbID <= 0 || season < 0 {
		return nil, errors.New("invalid args")
	}
	d, err := embyTMDBGetTVSeasonDetailAtLeast(database, tmdbID, season, minEpisodes)
	if err != nil {
		return nil, err
	}
	if d == nil {
		return []TMDBSeasonEpisode{}, nil
	}
...
func embyTMDBGetMovieDetailUpstream(database *db.DB, tmdbID int) (*embyTMDBMovieDetail, error) {
	...
	apiBase := tmdb.ResolveAPIBase(database)
	u, _ := url.Parse(tmdb.JoinAPI(apiBase, fmt.Sprintf("movie/%d", id)))
	params := u.Query()
	if lang != "" {
		params.Set("language", lang)
	}
...
func embyTMDBGetCredits(database *db.DB, mediaType string, tmdbID int) (*embyTMDBCredits, error) {
	...
	apiBase := tmdb.ResolveAPIBase(database)
	u, _ := url.Parse(tmdb.JoinAPI(apiBase, fmt.Sprintf("%s/%d/credits", typ, tmdbID)))
	params := u.Query()
	if strings.TrimSpace(lang) != "" {
		params.Set("language", strings.TrimSpace(lang))
	}
...
func embyTMDBGetPersonProfile(database *db.DB, personID int) (string, error) {
	...
	apiBase := tmdb.ResolveAPIBase(database)
	u, _ := url.Parse(tmdb.JoinAPI(apiBase, fmt.Sprintf("person/%d", personID)))
	params := u.Query()
	if strings.TrimSpace(lang) != "" {
		params.Set("language", strings.TrimSpace(lang))
	}
...
func smartTMDBGetTVDetail(database *db.DB, tmdbID int) (*embyTMDBTVDetail, error) {
	return embyTMDBGetTVDetail(database, tmdbID)
}

func smartTMDBGetMovieDetail(database *db.DB, tmdbID int) (*embyTMDBMovieDetail, error) {

...
type jellyfinDoubanHotItem struct {
	DoubanID string
	Title    string
	Year     int
	Rate     string
...
func jellyfinDoubanAPIBase(database *db.DB) (base string, proxyBase string) {
	if database == nil {
...
func jellyfinDoubanToProxiedURL(targetURL string, proxyBase string) string {
...
func jellyfinDoubanFetchRecentHot(database *db.DB, kind string, category string, hotType string, start int, limit int) ([]jellyfinDoubanHotItem, error) {
	...
	base, proxyBase := jellyfinDoubanAPIBase(database)
	...
	target := u.String()
	if proxyBase != "" {
		target = jellyfinDoubanToProxiedURL(target, proxyBase)
	}
	...
	if err != nil {
		jellyfinDebugPrintf("[jellyfin][douban] recent_hot %s %s %s -> request error: %v", k, category, hotType, err)
		return nil, err
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		jellyfinDebugPrintf("[jellyfin][douban] recent_hot %s %s %s -> http %d", k, category, hotType, resp.StatusCode)
		return nil, fmt.Errorf("douban http %d", resp.StatusCode)
	}

	body, err := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	if err != nil {
		jellyfinDebugPrintf("[jellyfin][douban] recent_hot %s %s %s -> read error: %v", k, category, hotType, err)
		return nil, err
	}
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		jellyfinDebugPrintf("[jellyfin][douban] recent_hot %s %s %s -> decode error: %v", k, category, hotType, err)
		return nil, err
	}
	if jellyfinDebugLogEnabled() && len(raw.Items) == 0 {
		snippet := strings.TrimSpace(string(body))
		if len(snippet) > 200 {
			snippet = snippet[:200]
		}
		ct := strings.TrimSpace(resp.Header.Get("Content-Type"))
		jellyfinDebugPrintf("[jellyfin][douban] recent_hot %s %s %s -> empty items (ct=%q) url=%q body=%q", k, category, hotType, ct, target, snippet)
	}

	out := make([]jellyfinDoubanHotItem, 0, len(raw.Items))
	for _, it := range raw.Items {
		...
		out = append(out, jellyfinDoubanHotItem{
			DoubanID: id,
			Title:    title,
			Year:     year,
			Rate:     rate,
	...
	if jellyfinDebugLogEnabled() && len(raw.Items) > 0 && len(out) == 0 {
		jellyfinDebugPrintf("[jellyfin][douban] recent_hot %s %s %s -> parsed 0/%d usable items (unexpected)", k, category, hotType, len(raw.Items))
	}
	return out, nil
}

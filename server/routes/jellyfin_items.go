...
func handleJellyfinItems(w http.ResponseWriter, r *http.Request, database *db.DB, serverID string, parts []string) {
	...
	if len(parts) >= 3 && strings.EqualFold(parts[1], "Images") && r.Method == http.MethodGet {
		...
		switch imgType {
		case "primary", "thumb":
			imgPath = jellyfinResolveItemImagePath(database, parsed, "primary", index)
			if imgPath == "" && imgType == "thumb" {
				imgPath = jellyfinResolveItemImagePath(database, parsed, "thumb", index)
			}
			size := "w500"
			if imgType == "thumb" {
				size = "w342"
			...
			http.Redirect(w, r, jellyfinTMDBImageURL(imgPath, size), http.StatusFound)
			return
		case "backdrop":
			imgPath = jellyfinResolveItemImagePath(database, parsed, "backdrop", index)
			if imgPath == "" {
				http.NotFound(w, r)
				return
			}
			http.Redirect(w, r, jellyfinTMDBImageURL(imgPath, "w1280"), http.StatusFound)
			return
		default:
			http.NotFound(w, r)
			return
...
func jellyfinResolveItemImagePath(database *db.DB, parsed *jellyfinItemID, kind string, index int) string {
	if parsed == nil {
		return ""
	}
	switch parsed.Kind {
	case "person":
		p, err := jellyfinTMDBGetPersonProfile(database, parsed.TMDBID)
		if err != nil {
			return ""
		}
		return p

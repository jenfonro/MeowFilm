...
type jellyfinDoubanTMDBMap struct {
...
func jellyfinGetDoubanTMDBMap(database *db.DB, kind string, doubanID string) (*jellyfinDoubanTMDBMap, error) {
	if database == nil {
...
func jellyfinUpsertDoubanTMDBMap(database *db.DB, m jellyfinDoubanTMDBMap) error {
	if database == nil {
...
func jellyfinResolveTMDBForDouban(database *db.DB, kind string, doubanID string, title string, year int) (tmdbID int, err error) {
	k := strings.TrimSpace(kind)
	id := strings.TrimSpace(doubanID)
	if k == "" || id == "" {
	...
	existing, err := jellyfinGetDoubanTMDBMap(database, k, id)
	if err != nil {
		return 0, err
	}
	...
	if q == "" {
		...
		_ = jellyfinUpsertDoubanTMDBMap(database, jellyfinDoubanTMDBMap{
			Kind:      k,
			DoubanID:  id,
			Title:     title,
			Year:      year,

...
func normalizeEmbyHomeSections(in []EmbyHomeSection) []EmbyHomeSection {
	list := make([]EmbyHomeSection, 0, len(in))
	...
	if len(list) == 0 {
		return DefaultEmbyHomeSections()
	}
	return list
}

func (d *DB) ReadEmbyHomeSections() ([]EmbyHomeSection, error) {
	if d == nil || d.db == nil {
		return DefaultEmbyHomeSections(), nil
	}
	var raw sql.NullString
	_ = d.db.QueryRow(`SELECT home_sections_json FROM app_emby WHERE id=1 LIMIT 1`).Scan(&raw)
	text := strings.TrimSpace(raw.String)
	if text == "" {
		return DefaultEmbyHomeSections(), nil
	}

	var out []EmbyHomeSection
	if err := json.Unmarshal([]byte(text), &out); err != nil {
		return DefaultEmbyHomeSections(), nil
	}

	return normalizeEmbyHomeSections(out), nil
}

func (d *DB) ReplaceEmbyHomeSections(sections []EmbyHomeSection) error {
	if d == nil || d.db == nil {
		return nil
	}
	normalized := normalizeEmbyHomeSections(sections)
	b, _ := json.Marshal(normalized)
	now := time.Now().Unix()
	_, _ = d.db.Exec(`
		INSERT INTO app_emby(id, home_sections_json, updated_at)
		VALUES(1, ?, ?)
